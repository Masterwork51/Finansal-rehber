...
const STATE = {
  ...
  rates: RATES,
  assets: [
    { id: 'a1', name: 'Banka TL', kind: 'bankAccount', currency: 'TRY', amount: 200000, accessibility: 'instant' },
    { id: 'a2', name: 'Nakit USD', kind: 'cash', currency: 'USD', amount: 1000, accessibility: 'instant' },
    { id: 'a3', name: 'Gram altın', kind: 'cash', currency: 'XAU', amount: 10, accessibility: 'month1plus' }
  ],
  incomes: [
    { id: 'i1', name: 'Maaş', amountTl: 85000, recurrence: 'regular', reliability: 'guaranteed', expectedDay: 15 },
    { id: 'i2', name: 'Kira geliri', amountTl: 10000, recurrence: 'regular', reliability: 'likely', expectedDay: 5 },
    { id: 'i3', name: 'Freelance', amountTl: 5000, recurrence: 'irregular', reliability: 'uncertain', expectedDay: null }
  ],
  expenses: [
...
test('Hızlı/yavaş likit ayrımı doğru', () => {
  const t = E.liquidTotals(STATE.assets, RATES);
  approx(t.quick, 246820);
  approx(t.slow, 45000);
  approx(t.total, 291820);
});
...
test('Zorunlu / yarı / isteğe bağlı kırılımı', () => {
  ...
  approx(t.essential, 31350);   // 22000 + 850 + 8500
...
test('Sadece garantili + düzenli + günü gelmemiş gelir sayılır', () => {
  const r = E.remainingIncomeThisMonth(STATE.incomes, DATE);
  approx(r.sum, 85000); // kira geliri (gün geçmiş) ve freelance (düzensiz) hariç
});

console.log('\nHedef rezervi:');
test('Hedefe kalan aya eşit bölünür', () => {
  const g = E.goalReserves(STATE.goals, DATE);
  approx(g.items[0].monthsLeft, 5); // Tem → Ara
  approx(g.sum, 12000);
});

console.log('\nAna hesap (bu ay, dengeli risk):');
test('Formül şeffaf şekilde tutuyor', () => {
...
test('Kötümser senaryo gerçekçiden düşük veya eşit', () => {
  ...
  assert.strictEqual(pess.riskLevel, 'conservative'); // dengeli → bir kademe konservatif
  approx(pess.expenseShock, 1.15);
});

test('Para yetmiyorsa sonuç 0 ve durum kırmızı', () => {
  const poor = { ...STATE, assets: [{ id: 'a', name: 'Az', kind: 'cash', currency: 'TRY', amount: 1000, accessibility: 'instant' }], incomes: [] };
...
test('Tampon yüzünden 0 ise durum turuncu (tight)', () => {
  // 10.000 TL + 85.000 maaş: giderleri ve hedefi karşılar ama yastığa yetmez → turuncu
  const mid = { ...STATE, assets: [{ id: 'a', name: 'Orta', kind: 'cash', currency: 'TRY', amount: 10000, accessibility: 'instant' }] };
...
test('Gelir katsayıları risk seviyesine göre uygulanır', () => {
  // dengeli: 85000 + 10000*0.85 + (düzensiz sayılmaz) = 93500
  approx(E.projectedMonthlyIncome(STATE.incomes, 'balanced'), 93500);
  // konservatif: 85000 + 10000*0.70 = 92000
  approx(E.projectedMonthlyIncome(STATE.incomes, 'conservative'), 92000);
});
